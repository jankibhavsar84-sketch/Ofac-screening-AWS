...
(async () => {
  ...
  const users=JSON.parse(fs.readFileSync('artifacts/perf/aws_perf_users.json','utf8'));
  ...
  const result = await page.evaluate(async ({users}) => {
    ...
    if (!buResp.ok) throw new Error(`business units load failed: ${buResp.status}`);
    ...
    for (const row of users) {
      const userId=String(row.username||'').trim();
      if (!userId) continue;
      const resp=await fetch(`/api/v1/admin/business-unit-mappings/${encodeURIComponent(userId)}`, {
        method:'PUT',
        headers,
        body: JSON.stringify({ user_name: userId, business_unit_codes: targetCodes }),
      });
      if (resp.ok) ok+=1;
      else failed.push({userId,status:resp.status,body:await resp.text()});
    }
