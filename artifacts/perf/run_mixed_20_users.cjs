...
const BATCH_FILE_PATH =
  process.env.BATCH_INPUT_FILE ||
  'C:\\Users\\Tapan Bhavsar\\Documents\\OFAC Screening Project\\sample_generated_populated_2603230001.xlsx';
const SCHEDULE_FILE_PATH =
  process.env.SCHEDULE_INPUT_FILE ||
...
const OUTPUT_PATH = path.resolve('artifacts', 'perf', `mixed_screening_20_users_${RUN_TAG}.json`);
...
function cohortForIndex(index) {
  if (index < 7) return 'batch';
  if (index < 14) return 'schedule';
  return 'single';
}
...
async function runBatch(page, index) {
  ...
  await fileInput.setInputFiles(BATCH_FILE_PATH);
...
async function runSchedule(page, index) {
  ...
  await fileInput.setInputFiles(SCHEDULE_FILE_PATH);
...
async function main() {
  if (!fs.existsSync(BATCH_FILE_PATH)) {
    throw new Error(`Batch input file not found: ${BATCH_FILE_PATH}`);
  }
  if (!fs.existsSync(SCHEDULE_FILE_PATH)) {
    throw new Error(`Schedule input file not found: ${SCHEDULE_FILE_PATH}`);
  }
  ...
  const summary = {
    base_url: BASE_URL,
    batch_input_file: BATCH_FILE_PATH,
    schedule_input_file: SCHEDULE_FILE_PATH,
